...
class FeatureInfoUtils  {
  ...
  buildFeatureInfoWMTSUrl(map, layer, viewResolution, coordinate, crs, format) {
    ...
    // Append TIME and any other dimensions set on the source via updateDimensions().
    // The Timeline component writes the current date via source.updateDimensions({ time: value }),
    // and getDimensions() returns exactly that object, so the GetFeatureInfo request
    // always uses the same temporal position as the visible tiles.
    const dimensions = source.getDimensions ? source.getDimensions() : {};
    Object.entries(dimensions).forEach(([key, value]) => {
      if (value != null && value !== '') url += '&' + encodeURIComponent(key) + '=' + encodeURIComponent(value);
    });
  ...
  buildFeatureInfoWMTSXYZUrl(map, layer, viewResolution, coordinate, crs, format) { 
    ...
    // Append TIME and any other dimensions set on the source via updateDimensions().
    // Mirrors the fix in buildFeatureInfoWMTSUrl — ensures WMTSXYZ layers also
    // send the correct temporal position in GetFeatureInfo requests.
    const wmtsxyzSource = layer.getSource ? layer.getSource() : null;
    const xyzDimensions = (wmtsxyzSource && wmtsxyzSource.getDimensions) ? wmtsxyzSource.getDimensions() : {};
    Object.entries(xyzDimensions).forEach(([key, value]) => {
      if (value != null && value !== '') url += '&' + encodeURIComponent(key) + '=' + encodeURIComponent(value);
    });
